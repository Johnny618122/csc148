    ...
    def apply(self, customers: List[Customer],
        ...
        try:
            ...
            for customer in customers:
                if filter_id == customer.get_id():
                    calls_list.extend(customer.get_history()[0])
                    calls_list.extend(customer.get_history()[1])
                    return calls_list
            return data
        except:
            return data
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        try:
            call_list = []
            coord_list_string = [x.strip() for x in filter_string.split(',')]
            coord_list = []

            for coord in coord_list_string:
